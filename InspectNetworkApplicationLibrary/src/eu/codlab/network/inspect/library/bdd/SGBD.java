...
{    
	 ...
	 * 
	 */
	static final String CREATE_INTERFACES = "create table if not exists "+TABLE_INTERFACES+" (id integer primary key autoincrement, name text, up integer)";
	static final String CREATE_INTERFACES_DATA = "create table if not exists "+TABLE_INTERFACES_DATA+" (id integer primary key autoincrement, interface_id integer, timestamp integer, up integer, down integer)";
	...
	public long addInterfaces(String name, boolean up){
	...
	public Interface [] getInterfaces(){
		...
		if(mCursor.getCount()>0){
			...
			while(!mCursor.isAfterLast()){
				...
				up =mCursor.getInt(mCursor.getColumnIndex("up")) > 0;
				mCursor.moveToNext();
				names[index] = new Interface(id, name, up);
				index++;
			}
		}
		mCursor.close();
	...
	public long addData(long interface_id, long up, long down, long timestamp){
		ContentValues initialValues = new ContentValues();
