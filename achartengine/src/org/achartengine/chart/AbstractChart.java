...
public abstract class AbstractChart implements Serializable {
    ...
    public int getBottomGraph() {
        return bottom;
    }

    protected int[][] titlesX;

    public int[][] getTitlesXY() {
        return titlesX;
    }
        ...
        if (renderer.isShowLegend()) {
            ...
            for (int i = 0; i < sLength; i++) {
                ...
                if (i > 0 && getExceed(currentWidth, renderer, right, width)) {
                    currentX = left;
                    currentY += renderer.getLegendTextSize() + 10;
                ...
                titlesX[i][0] = (int) (currentX);
                titlesX[i][1] = (int) (currentY);//-size);
                currentX += extraSize;
            }
        }
    ...
    private String getNormalized(double value) {
        ...
        if (value > 1000000000) {
            ...
            return "  " + (Math.round(value / 10) * 1. / 100) + "K";
        } else {
            return "  " + value;
        }
    }
    ...
    protected String getLabel(double label) {
        String text = "";
        double rounded = Math.round(label);
        if (label == rounded) {
            text = getNormalized(rounded);
        } else {
            text = getNormalized(label);
        }
        return text;
    }
    ...
    protected void drawPath(Canvas canvas, float[] points, Paint paint, boolean circular) {
        ...
        //deplacement sur x,y x',y
        path.moveTo(tempDrawPoints[0], tempDrawPoints[1]);
        path.lineTo(tempDrawPoints[2], tempDrawPoints[1]);
        //deplacement sur x',y puis sur x',y'
        path.moveTo(tempDrawPoints[2], tempDrawPoints[1]);
