...
public abstract class XYChart extends AbstractChart {
    ...
    public void setAutoAdapt(boolean adapt) {
        mAdapt = adapt;
    }
    ...
    public void draw(Canvas canvas, int x, int y, int width, int height, Paint paint) {
        ...
        int[] paddings = mRenderer.getPaddings();
        ...
        for (int i = 0; i < sLength; i++) {
            ...
            if (!(series.getItemCount() == 0 || series.isInvisible())) {
                if (!isMinXSet[scale]) {
        ...
        if (isAutoAdapt()) {
            ...
            for (int i = 0; i < sLength; i++) {
                ...
                for (Entry<Double, Double> value : range.entrySet()) {
                    maxY[scale] = Math.max(value.getValue(), maxY[scale]);
                    minY[scale] = Math.min(value.getValue(), minY[scale]);
                }
            }
        }
        ...
        for (int i = 0; i < maxScaleNumber; i++) {
            ...
            if (maxY[i] - minY[i] != 0) {
                yPixelsPerUnit[i] = (float) ((bottom - paddings[2] - (top + paddings[0])) / (maxY[i] - minY[i]));
            }
        }
        ...
        for (int i = 0; i < sLength; i++) {
            ...
            float yAxisValue = Math.min(bottom - paddings[2], (float) (bottom - paddings[2] + yPixelsPerUnit[scale] * minY[scale]));
            ...
            for (Entry<Double, Double> value : range.entrySet()) {
                ...
                if (!isNullValue(yValue)) {
                    ...
                    points.add((float) (bottom - paddings[2] - yPixelsPerUnit[scale] * (yValue - minY[scale])));
                } else if (isRenderNullValues()) {
                    points.add((float) (left + xPixelsPerUnit[scale] * (xValue - minX[scale])));
                    points.add((float) (bottom - paddings[2] - yPixelsPerUnit[scale] * (-minY[scale])));
                } else {
                    if (points.size() > 0) {
        ...
        if (showLabels || showGridX) {
            List<Double> tmp = getXLabels(minX[0], maxX[0], mRenderer.getXLabels());
            List<Double> xLabels = new ArrayList<Double>();
            if (tmp.size() > 0) {
                if (tmp.get(0) < minX[0]) {
                    for (int i = 0; i < tmp.size(); i++) {
                        xLabels.add(tmp.get(i) + minX[0]);
                    }
                } else {
                    for (int i = 0; i < tmp.size(); i++) {
                        xLabels.add(tmp.get(i));
                    }
                }
            }
    ...
    protected void drawYLabels(Map<Integer, List<Double>> allYLabels, Canvas canvas, Paint paint,
                               int maxScaleNumber, int left, int right, int bottom, double[] yPixelsPerUnit, double[] minY, float padding_left, float padding_right) {
        Orientation or = mRenderer.getOrientation();
        boolean showGridX = mRenderer.isShowGridX();
        boolean showLabels = mRenderer.isShowLabels();
        for (int i = 0; i < maxScaleNumber; i++) {
            ...
            for (int j = 0; j < length; j++) {
                ...
                if (or == Orientation.HORIZONTAL) {
                    if (showLabels && !textLabel) {
                        ...
                        if (axisAlign == Align.LEFT) {
                            canvas.drawLine(left + getLabelLinePos(axisAlign), yLabel, left, yLabel, paint);
                            drawText(canvas, getLabel(label), left + 20 + padding_left, yLabel - 2, paint,
                                    mRenderer.getYLabelsAngle());
                        } else {
                            canvas.drawLine(right, yLabel, right + getLabelLinePos(axisAlign), yLabel, paint);
                            drawText(canvas, getLabel(label), right - padding_right, yLabel - 2, paint,
                                    mRenderer.getYLabelsAngle());
                        }
                    }
                    if (showGridX) {
        ...
        if (showLabels) {
            ...
            for (Double location : xTextLabelLocations) {
                if (minX <= location && location <= maxX) {
                    ...
                    paint.setColor(0xff000000);
                    drawText(canvas, mRenderer.getXTextLabel(location), xLabel,
                            (bottom - top) / 2 + mRenderer.getLabelsTextSize() * 4 / 3, paint, 0);//mRenderer.getXLabelsAngle());
